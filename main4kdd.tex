...
\input{generated/appendix_confirmatory}
...
\]
with $m=64$ cells. A candidate control anchor $u$ defines a control patch $P(u)$ in the same way. Candidate anchors are selected from non-perturbed or control-like cells within the same data partition. The implementation nominally prefers anchors with the same coarse cell type as $a$; however, all coarse labels in the reported cache mapped to one effective ID, so this filter is constant. Partitioning prevents any post anchor, matched-control patch, or derived pseudo-target from crossing train, validation, and test partitions.

Each candidate control patch is scored by spatial proximity, center-cell transcriptional similarity, patch-level transcriptional similarity, and cell-type composition. The implementation first retains at most the 256 spatially nearest candidates. It scales spatial distance by
\[
\eta_a=\max\left\{\operatorname{median}_{u}
\|s_a-s_u\|_2,\,100\right\},
\]
while the three other channels remain mean-squared distances in their native
feature spaces. The implemented matching score is
\[
\mathrm{score}(a,u)
=
0.35\,\frac{d_{\mathrm{sp}}(a,u)}{\eta_a}
+
0.35\,d_{\mathrm{ctr}}(a,u)
+
0.20\,d_{\mathrm{patch}}(a,u)
+
0.10\,d_{\mathrm{ctype}}(a,u),
\]
with raw channels
\[
\begin{aligned}
...
In the reported primary cache, $d_{\mathrm{ctype}}=0$ for every candidate
because of the collapsed label map. Thus the effective matching score uses
spatial, center-expression, and patch-expression channels; the composition
term contributes no ranking information.
...
\end{aligned}
\]
The same provenance issue makes the cell-type mismatch penalty constant in
this cache. We retain it in the general formulation but do not attribute any
reported result to cell-type-aware transport.
...
\caption{\textbf{Historical reference-ST adapter construction.} Candidate
tissue regions are scored for local geometry, cell-state organization, and
communication compatibility. These regions were used in an optional
development-stage adapter without perturbation labels. The paired low-data
audit in \Cref{tab:app_reference_st_lowdata} finds near-zero response-field
gain, so this adapter is not part of core \methodname{}.}
\label{fig:app_st_mining}
\end{figure*}

\subsection{Historical communication-rich microenvironment mining}
...
An optional development-stage experiment used unperturbed high-resolution
spatial transcriptomics to identify reference microenvironments with strong
local structure. These data were never used as perturbation labels. We retain
the full construction for reproducibility, although confirmatory paired tests
show no robust gain and the adapter is excluded from core \methodname{}.
...
In the original development pipeline, the mined regions pretrained a
reference microenvironment encoder. They expose the optional adapter to local
gene coupling but do not define perturbation-response targets. Spatial
Perturb-Seq \ppp patches remain the only signed response-field supervision.
...
\end{aligned}
\]
where $\mathcal G$ is the set of communication programs and $\beta_{pq}$ is a
program weight learned from the mined regions. This score was evaluated as an
optional edge feature. It does not define the perturbation target, and
randomizing cached communication scores leaves Ring Pearson unchanged
(0.4574 versus 0.4575); we therefore make no performance claim for this prior.

\subsection{Mouse reference-ST mining used by the optional adapter}
...
These anchor--normal pairs were used only to pretrain the optional reference
adapter. Signed response targets still come exclusively from Spatial
Perturb-Seq \ppp pairs. The scale of this corpus documents the attempted
extension, not evidence that reference-ST pretraining improves perturbation
prediction; paired low-data gains are near zero.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{mouse_mining.png}
\vspace{-0.6em}
\caption{\textbf{Mouse-brain reference-ST mining for the optional adapter.}
Program-enriched regions from unperturbed HD and Xenium data are paired with
matched background regions. They do not define perturbation-response targets,
and the resulting adapter is excluded from core \methodname{} after the
negative paired-gain audit.}
...
\paragraph{Context residualization.}
The canonical predictive decomposition is
\[
\widehat\Delta_i=\widehat C_i+\widehat R_i,\qquad
\widehat C_i=f_{\mathrm{context}}(x,c,p,e;\,a=0,d=0).
\]
The context model is trained first and frozen. The residual target is
\[
R_i^\star=\Delta_i^{\mathrm{PPP}}-\widehat C_i .
\]
This is a predictive separation and does not identify causal communication.

\paragraph{Direct-effect and receiver initialization.}
The baseline embedding and separate receiver/direct-effect states are
\[
\begin{aligned}
...
\end{aligned}
\]
The profile $d_i$ is zero at receiver cells, so they cannot directly read the
perturbation input.

\paragraph{Implemented direct-effect-to-receiver propagation.}
The propagation block filters the graph to edges with a direct-effect source
and receiver destination. For each retained edge,
\[
\begin{aligned}
z_{ij}^{\ell}
&=\sigma\!\left(
...
\end{aligned}
\]
Direct-effect states remain fixed across layers. Receiver states are updated
only by direct-effect-to-receiver messages; the core code contains no
receiver-to-receiver path. A receiver head decodes $\widehat R_j$ from
$b_j$, $n_j^L$, and a graph-level sender summary.

\paragraph{Optional edge and refinement diagnostics.}
The communication score $q_{ij}$ is an optional gate bias. Randomizing it gives
Ring Pearson 0.4575 versus 0.4574 for the unmodified cache. The released code
also contains optional response-feedback and reference-ST adapters. The
distance-aware response-feedback variant groups first-pass responses into
three rings and applies a restricted second correction. The unperturbed
reference-ST adapter is excluded from core \methodname{} because its paired
low-data gains are near zero (\Cref{tab:app_reference_st_lowdata}).

\paragraph{Training objective and inference.}
The implemented residual objective is
\[
...
\]
The terms are high-confidence receiver Huber loss, ring-summary loss,
post-state reconstruction, and pairwise gene-response ranking. The
direct-effect-cell loss weight is zero. At inference, the frozen context and
residual predictions are added, and post-state expression is reconstructed as
$\widehat x_i^{\mathrm{post}}=x_i+\widehat\Delta_i$.

\subsection{Gene-centric neighbor-response validation status}
\label{app:full_gene_centric_validation}

The held-out benchmark predicts 512 genes. We therefore report the complete
receiver-side gene distribution rather than only selected markers.
\Cref{tab:app_full_gene_distribution} shows that performance is heterogeneous:
receiver-gene Pearson has mean/median $0.304/0.146$, with a p10--p90 range of
$0.057$--$0.860$. Pearson is positive for $99.0\%$ of genes, exceeds $0.2$ for
$38.9\%$, and exceeds $0.5$ for $24.2\%$. Per-gene $R^2$ is positive for
$53.7\%$ of genes and has median $0.004$. The Spearman association between
mean absolute response magnitude and receiver-gene Pearson is $0.910$.
Accordingly, the high aggregate cell--gene correlation should not be
interpreted as uniformly strong recovery of weak-response genes.
...
\caption{\textbf{Complete held-out receiver-gene distribution.}
All 512 output genes are included; no marker filtering is applied.}
\label{tab:app_full_gene_distribution}
\begin{tabular}{lrrrrr}
\toprule
\textbf{Metric} & \textbf{p10} & \textbf{p25} & \textbf{Median} & \textbf{p75} & \textbf{p90} \\
\midrule
Pearson $r$ & 0.057 & 0.090 & 0.146 & 0.457 & 0.860 \\
$R^2$ & -0.047 & -0.024 & 0.004 & 0.174 & 0.730 \\
Sign accuracy & 0.230 & 0.330 & 0.468 & 0.660 & 0.843 \\
\bottomrule
\end{tabular}
\end{table}
...
To test perturbation-level generalization, we use a strict grouped guide-out
split. The perturbation guides \textit{fasn}, \textit{stk39}, and
\textit{trem2} are jointly excluded from training and validation, yielding 992
unseen-guide graphs. With observed sender-side direct-effect seeds,
\methodname{} reaches pooled neighbor/ring Pearson $0.596/0.451$. A strict
context-only node MLP reaches $0.818/0.654$, confirming that absolute PPP field
correlation remains context dominated. The relevant \methodname{} diagnostic is
the measured-versus-zero-seed Ring lift ($0.451$ versus $0.437$), not
total-field superiority. Per-guide \methodname{} performance remains stable
across all three held-out perturbations.
...
The \methodname{} rows are consistent across the three unseen perturbations:
observed-seed neighbor Pearson stays between $0.5978$ and $0.6006$, and
observed-seed ring Pearson stays between $0.4540$ and $0.4573$. Zero-seed
evaluation preserves nearly identical neighbor Pearson but reduces pooled Ring
Pearson from $0.4514$ to $0.4374$. The context-only node MLP nevertheless
dominates absolute field prediction, so this table supports only a small,
stable perturbation-conditioned Ring lift rather than overall guide-out
superiority.

\paragraph{Five-fold guide OOD and cross-assay seed transfer.}
The original three-guide split above is fold 0 of a grouped five-fold audit in
which every guide is assigned to test exactly once. Measured spatial seeds
improve Ring in all five folds, whereas validation-calibrated guide profiles
from the independent companion scRNA-seq assay underperform zero seeds in
every fold (\Cref{tab:app_fivefold_seed_transfer}).

\begin{table}[t]
\centering
\footnotesize
\caption{\textbf{Context-only Ring correlation across grouped-guide folds.}
Seed-conditioned results for the same folds are reported in
\Cref{tab:app_fivefold_seed_transfer}.}
\label{tab:app_fivefold_context}
...
This figure provides spatial response maps, local receiver neighborhoods, and gene-level recovery patterns under the block-held-out matched-control setting.}
...
\begin{table}[t]
\centering
\caption{\textbf{Development-stage propagation-blend sensitivity.}
$\alpha=0$ is the base propagation module and $\alpha=1$ is the historical
reference-ST ring-refinement output. This post-hoc sweep is retained for
provenance; component attribution uses the paired low-data audit.}
...
The post-hoc development sweep selects $\alpha=0.65$ by its composite score,
with only small changes across settings. Because the sweep is not an
independent paired estimate of the adapter contribution, we do not interpret
the difference as evidence that reference-ST pretraining improves the field.
...
The frozen reference encoder fits its self-supervised objectives on
unperturbed reference ST. These diagnostics are representation checks only:
they neither establish perturbation-response accuracy nor contradict the
near-zero paired response-field gains.
...
These development ablations support context residualization and
direct-effect-to-receiver routing. The ``no edge attributes'' intervention
removes geometry and nominal pair features together with communication scores,
so it cannot attribute the decrease specifically to communication priors; the
targeted randomized-CCC control shows no measurable CCC contribution.
...
The largest development gain comes from explicit
direct-effect-to-receiver residual routing over the context model. Optional
second-pass and reference-adapter changes are small and inconsistent across
metrics; the independent low-data audit therefore supersedes any positive
reference-ST interpretation. Including direct-effect-cell reconstruction
substantially hurts receiver propagation, consistent with the easier sender
task dominating optimization. Removing ring loss produces a small
ring-structure decrease.
...
\caption{\textbf{De novo IFN-$\gamma$ response-field hypothesis in mouse
brain.} Whole-tissue response maps, six high-response regions, and local
64-cell neighborhoods illustrate heterogeneous IFN/MHC-I-associated and
structural-program responses. This deployment uses an externally predicted
direct-effect profile and has no paired spatial IFN-$\gamma$ ground truth; it
is therefore a plausibility example rather than validation or discovery.}
\label{fig:app_ifng_response_field}
\Description{Whole-tissue and local-neighborhood maps of a predicted mouse-brain
IFN-gamma response field, including representative immune-response and
structural genes.}
\end{figure*}
...
Frozen reference prior used for deployment; no matched IFN-$\gamma$ labels or paired prior ablation. \\
\bottomrule
\end{tabularx}
\end{table}
...
The 64-nearest-neighbor union around 2,048 top-5\% targets covers 38,922 of
40,944 cells in the analyzed bottom-right region (95.06\%). Patch-level
gradients are directionally heterogeneous: the $x$ coordinate correlates
negatively with \textit{B2m} and \textit{Irf8} response
($r=-0.290$ and $-0.296$) and positively with \textit{Col1a2} and
\textit{Flt1} response ($r=0.307$ and $0.417$). We omit cell-level
significance tests because spatial autocorrelation and overlapping patches
violate independent-sample assumptions.

\paragraph{Role of the reference microenvironment prior.}
This deployment used the historical frozen reference adapter, but there is no
paired IFN-$\gamma$ spatial target or matched adapter ablation. Its contribution
cannot be identified from this example. The case is therefore a
hypothesis-generating interface demonstration, not validation of either the
adapter or IFN-$\gamma$ biology.
...
The optional reference-ST encoder was pretrained without perturbation labels.
Although retained in the released development code and deployment example, it
does not show robust paired Ring gain and is not part of core \methodname{}.
...
\subsection{Exploratory discovery-set prioritization audit}
\label{app:selector_audit}

Beyond predicting spatial response fields, we asked whether frozen
\methodname{} outputs can prioritize perturbations: among candidate actions
that preserve a specified target program in a spatial ring, select an
alternative with lower collateral burden. The figures and detailed operating
points below preserve the original discovery audit. Its legacy
``validation'' cache duplicated training graphs and its method family was
examined post hoc, so the $0.354$--$0.418$ reductions are exploratory rather
than the confirmatory main result. The independent three-way audit in
\Cref{tab:selector_main} selects the complete rule on a disjoint validation
block and obtains TQR $0.694$ with $0.120$ mean relative reduction on frozen
test. Neither audit is prospective validation.

The intervention selector scores each candidate by target utility and
collateral burden. Target utility measures whether the desired program response
is retained in the requested spatial ring. Burden summarizes far-field,
off-program, and validation-defined undesirable-program components.
\Cref{fig:app_selector_prioritization} gives the full exploratory audit. Across
5,286 actions and 36 target specifications, the examined operating points reach
TQR 0.620--0.667 and post-hoc burden reduction 0.354--0.418. The observed-field
oracle reaches 0.672. These discovery-set values characterize mechanism and
oracle gap, not held-out selector effect size.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{selector_prioritization_appendix.pdf}
\caption{\textbf{Detailed legacy discovery audit for perturbation
prioritization.}
...
(c) Burden decomposition across all three cases shows weighted, far-field,
off-program, and validation-defined undesirable-program burden reductions.
...
(f) Predicted target utility stratifies observed target qualification, but
absolute calibration remains imperfect under both main-block and strict-guide
splits.}
\label{fig:app_selector_prioritization}
\end{figure*}

Three exploratory biological cases give the most interpretable view of the
legacy audit (Table~\ref{tab:app_selector_cases}). The selected alternatives
retained 89.3--99.8\% of target utility while reducing collateral burden by
24.4--49.7\%. For TGF-$\beta$/stroma near-down, the selector replaced
\texttt{sgrna\_rraga} with \texttt{sgrna\_oligo2}, preserving target score
from 5.074 to 4.968 while reducing weighted burden from 1.188 to 0.597. The
same case reduced far-field burden from 1.213 to 0.604, off-program burden
from 1.228 to 0.615, and validation-defined undesirable-program burden from 1.457
to 0.778 (Table~\ref{tab:app_selector_burden_decomposition}).
Vascular-stress near-down and myeloid-activation far-up show the same
target-retaining, lower-burden pattern; these examples illustrate biological
interpretability and are not used to estimate out-of-sample effect size.

\begin{table}[t]
\centering
\scriptsize
\caption{Exploratory response-field prioritization cases from the legacy
discovery audit.}
...
\toprule
Target & Role & Target score & Weighted burden & Far-field & Off-program & Undesirable \\
\midrule
...
\centering
\small
\caption{Selector operating points for the retrospective audit.}
...
Perturb-FISH provides external support for the direction of these
response-field prioritization cases, but we interpret it as module-level
support rather than full selector replication. The action pool and readout
panel differ from Spatial Perturb-seq, so the external assay tests whether the
same module-level target and burden directions are plausible rather than
whether the identical guide-level selector transfers unchanged. On 39
validation-defined reliable Perturb-FISH specifications, the full
predicted-utility/predicted-burden selector reached TQR 0.718, burden reduction
0.151, observed-utility/predicted-burden reduction 0.207, and global
pairwise lower-burden agreement 0.851
(Table~\ref{tab:app_selector_perturb_fish}).
...
\midrule
Reliable specifications & 39 & 0.718 & 0.151 & 0.207 & 0.874 \\
All specifications & 64 & 0.641 & 0.137 & 0.218 & 0.851 \\
\bottomrule
\end{tabular}
\end{table}

Finally, the audit has a calibration limitation. Predicted utility deciles
stratify observed target qualification, but absolute reliability is imperfect:
the main block split rises from TQR 0.043 in the bottom predicted-utility decile
to 0.575 in the top decile, while strict guide holdout rises from 0.053 to
0.461 (Table~\ref{tab:app_selector_calibration}). We therefore report this as a
retrospective response-field prioritization audit rather than a calibrated
controller for prospective intervention design.

\begin{table}[t]
\centering
\small
\caption{Target-utility calibration limitation. Target-qualified rate (TQR)
increases from the lowest to highest predicted-utility decile, but absolute
calibration remains imperfect, especially under strict guide holdout.}
...
For reproducibility of the optional negative diagnostic, the reference-ST
adapter uses patch size 64, edge $k=8$, local CCC top-$k=5$, 4096/512/512
train/validation/test samples, hidden dimension 128, batch size 32, 20 epochs,
and learning rate $10^{-3}$. The historical organ-ring refinement uses batch
size 16: the organ-adapter stage runs for 10 epochs at $5\times10^{-4}$, the
response-refine stage for 6 epochs at $2\times10^{-4}$, the ring-refine stage
for 6 epochs at $10^{-4}$, and the final organ-ring stage for 6 epochs at
$7.5\times10^{-5}$. The development blend sweep uses batch size 16 and
$\alpha\in\{0.0,0.25,0.5,0.55,0.6,0.65,0.7,0.75,1.0\}$.
