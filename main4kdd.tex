...
The held-out benchmark predicts 512 genes. The current appendix reports
aggregate gene-space metrics, response-magnitude-stratified examples, and the
pre-specified perturbation-related genes used in the original analysis.
Exporting the complete per-gene percentile table remains a submission
checklist item; selected markers should not be interpreted as the full
...
\subsection{Retrospective spatially constrained perturbation prioritization}
\label{app:selector_audit}

Beyond predicting spatial response fields, we asked whether frozen \methodname{} response-field outputs can prioritize perturbations: among candidate actions that preserve a specified target program in a spatial ring, select an alternative with lower collateral burden. We use this appendix only as a retrospective audit of response-field utility. Candidate actions and target specifications were fixed before held-out evaluation, and observed fields were used only for evaluation. Because the audit was assembled from frozen response-field caches compatible with the \methodname{} interface, we do not treat it as prospective validation.

The intervention selector scores each candidate by target utility and collateral burden. Target utility measures whether the desired program response is retained in the requested spatial ring. Burden summarizes responses outside the requested target, including far-field, off-program, and pre-specified undesirable-program components. \Cref{fig:app_selector_prioritization} gives the full audit. Across 5,286 candidate perturbation actions and 36 target specifications, a clean weighted-head operating point reached a target-qualified rate (TQR) of 0.639 and burden reduction of 0.354. An off-program-sensitive operating point reached TQR 0.667 and burden reduction 0.362, while a rank-consensus operating point increased burden reduction to 0.418 at TQR 0.620. These values remain below the observed-field oracle burden reduction of 0.672, as expected for a predicted-field selector.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{selector_prioritization_appendix.pdf}
\caption{\textbf{Detailed retrospective perturbation-prioritization audit.}
...
(c) Burden decomposition across all three cases shows weighted, far-field, off-program, and pre-specified undesirable-program burden reductions.
