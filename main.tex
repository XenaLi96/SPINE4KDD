...
\noindent\textbf{Masked niche module.}
...
\begin{aligned}
\mu_{ij}^{\ell}
&= \sigma\!\left(\phi_a([u_i^0,n_j^\ell,e_{ij}]) + w_{\mathrm{int}}q_{ij}\right)
...
\noindent\textbf{Reference-ST niche encoder.}
...
\noindent\textbf{Distance-aware refinement.}
...
&\quad + \alpha\widehat I_i^{\mathrm{prop},\mathrm{st}},
\qquad \alpha=0.65 .
\end{aligned}
\end{equation}
...
\subsection{Application: Using SPINE as a Drug Selector}
\label{sec:drug_selector_application}
Spatial response fields can also be used as a downstream drug/perturbation selector. Instead of evaluating only one perturbation query, we score each candidate action by a target utility \(U\), measuring whether the requested program response is retained in a specified spatial ring, and a collateral burden \(S\), measuring far-field, off-program, and harmful-program responses outside the target. The selector ranks only target-retaining actions and then prefers lower-burden alternatives. This use is a retrospective response-field audit rather than a prospective controller, because observed held-out fields are used only for evaluation.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{selector_application_main.pdf}
\caption{\textbf{Using \methodname{} response fields as a drug selector.}
(a) \methodname{} response fields are converted into target utility and collateral burden scores, then used to rank target-retaining, low-burden actions.
(b) Three pre-specified biological cases keep target utility while moving away from the target-only baseline toward lower burden.
(c) In the TGF-$\beta$/stroma near-down case, the selected action lowers weighted, far-field, off-program, and harmful burden.
(d) Retrospective support combines the Spatial Perturb-seq selector audit and Perturb-FISH module-level support, while target-utility calibration remains bounded.}
\label{fig:drug_selector_application}
\end{figure}

In the Spatial Perturb-seq audit, the selector screened 5,286 candidate actions across 36 target specifications. A clean weighted operating point reached TQR 0.639 with burden reduction 0.354, and an off-program-sensitive operating point reached TQR 0.667 with burden reduction 0.362. A rank-consensus operating point increased burden reduction to 0.418 at TQR 0.620. Three biological examples retained 89.3--99.8\% of target utility while reducing collateral burden by 24.4--49.7\%. The strongest example, TGF-$\beta$/stroma near-down, reduced weighted burden from 1.188 to 0.597 while preserving target score from 5.074 to 4.968. External Perturb-FISH support is directionally consistent at the module level: on 39 reliable specifications, the predicted-utility/predicted-burden selector reached TQR 0.718, burden reduction 0.151, and pairwise safety 0.851. These results suggest that \methodname{} response fields can help prioritize interventions that preserve a desired local target response while reducing spatial collateral effects. Full operating points, burden decompositions, Perturb-FISH support, and calibration limits are provided in \Cref{app:selector_audit,fig:app_selector_prioritization}.
\FloatBarrier
...
(b) Three pre-specified biological cases show high target retention and lower collateral burden.
(c) Burden decomposition across all three cases shows weighted, far-field, off-program, and harmful-program burden reductions.
...
\centering
\scriptsize
\caption{Pre-specified response-field prioritization cases.}
...
\toprule
Subset & Specs & TQR & Burden & Obs-U & Safety \\
\midrule
