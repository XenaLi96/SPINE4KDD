...
\methodname{} predicts spatial perturbation response fields from an unperturbed cell-resolved tissue graph and a direct-effect seed. The input graph contains matched-control expression, cell-state labels, relative coordinates, local geometry, direct-effect indicators, and edge features that encode spatial distance, cell-state-pair identity, and communication-compatibility scores. The model separates the response into two terms. A masked niche branch removes perturbation inputs and estimates the component explained by local tissue context, while a directed propagation branch injects perturbation information only through direct-effect cells and routes gated messages to neighboring cells. A frozen reference-ST niche encoder supplies perturbation-label-free microenvironment priors, and a distance-aware refinement module summarizes high-response neighbors across radial rings before a second propagation pass. The final output is a signed cell-by-gene response delta, which is added to the matched-control state to reconstruct the predicted post-perturbation expression field.}
...
&= a_i\psi_{\mathrm{direct}}(u_i^0)\\
&\quad + (1-a_i)\psi_{\mathrm{neighbor}}(n_i^L).
\end{aligned}
\end{equation}
