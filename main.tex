...
\methodname{} is intended for hypothesis generation and benchmarked propagation analysis, not for clinical decision making or unsupervised therapeutic deployment. A key technical limitation is that PPP targets are guide-consistent pseudo-targets rather than directly observed causal pre/post measurements of the same cells; therefore, the de novo IFN-$\gamma$ case study requires direct experimental validation before any biological conclusion is acted on. Spatial and single-cell perturbation data also raise privacy concerns, since molecular profiles, tissue locations, disease labels, and donor metadata can remain sensitive after de-identification. Released artifacts will use public or de-identified data products and split manifests, and any future human-data extension should follow the relevant consent, IRB, and data-use terms. Current benchmarks are biased toward mouse brain, selected perturbations, available cell states, and public spatial assays, so performance may not transfer to other organs, species, disease states, or rare cell populations. PPP matching and reference-ST mining may further bias the model toward abundant cell types and well-represented microenvironments. Potential misuse includes prioritizing perturbations without wet-lab validation, overinterpreting pseudo-targets as causal effects, or applying response-field predictions in sensitive disease contexts without oversight. These limitations motivate conservative interpretation, transparent release, and prospective validation before using \methodname{} to guide intervention design.
