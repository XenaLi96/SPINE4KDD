...
The SPINE drug selector scores each candidate by target utility and collateral burden. Target utility measures whether the desired program response is retained in the requested spatial ring. Burden summarizes responses outside the requested target, including far-field, off-program, and harmful-program components. Fig.~\ref{fig:app_selector_prioritization} summarizes the selector workflow and main diagnostic panels. Across 5,286 candidate perturbation actions and 36 target specifications, a clean weighted-head operating point reached a target-qualified rate (TQR) of 0.639 and burden reduction of 0.354. An off-program-sensitive operating point reached TQR 0.667 and burden reduction 0.362, while a rank-consensus operating point increased burden reduction to 0.418 at TQR 0.620. These values remain below the observed-field oracle burden reduction of 0.672, as expected for a predicted-field selector.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{selector_prioritization_appendix.pdf}
\caption{\textbf{Retrospective SPINE drug-selector audit.}
(a) Frozen \methodname{} response fields are scored as target utility and collateral burden, then used as a drug/perturbation selector that ranks only target-retaining actions.
(b) Three pre-specified biological cases move from the target-only baseline toward lower burden while mostly retaining target utility.
(c) For the TGF-$\beta$/stroma near-down case, the selected alternative lowers weighted, far-field, off-program, and harmful burden components.
(d) The same selector signal is summarized with spatial-audit and Perturb-FISH support metrics, while predicted target utility still requires calibration under strict guide holdout.}
