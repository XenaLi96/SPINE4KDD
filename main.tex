...
\subsection{Retrospective response-field prioritization audit}
\label{app:selector_audit}

Beyond predicting spatial response fields, we asked whether frozen response-field outputs can support a downstream biological prioritization task: among perturbations that preserve a specified target program in a spatial ring, select an alternative with lower collateral burden. We use this appendix only as a retrospective audit of response-field utility. Candidate actions and target specifications were fixed before held-out evaluation, and observed fields were used only for evaluation. Because the audit was assembled from frozen response-field caches compatible with the \methodname{} interface, we do not treat it as a second main method or as prospective validation.

The audit scores each candidate by target utility and collateral burden. Target utility measures whether the desired program response is retained in the requested spatial ring. Burden summarizes responses outside the requested target, including far-field, off-program, and harmful-program components. Fig.~\ref{fig:app_selector_prioritization} summarizes the audit design and main diagnostic panels. Across 5,286 candidate perturbation actions and 36 target specifications, a clean weighted-head operating point reached a target-qualified rate (TQR) of 0.639 and burden reduction of 0.354. An off-program-sensitive operating point reached TQR 0.667 and burden reduction 0.362, while a rank-consensus operating point increased burden reduction to 0.418 at TQR 0.620. These values remain below the observed-field oracle burden reduction of 0.672, as expected for a predicted-field selector.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{selector_prioritization_appendix.pdf}
\caption{\textbf{Retrospective response-field prioritization audit.}
(a) Audit schematic: frozen response fields are decomposed into target utility and collateral burden, then used to select lower-burden target-retaining alternatives.
(b) Three pre-specified biological cases retain most target utility while reducing burden.
(c) TGF-$\beta$/stroma near-down burden decomposition; the selected alternative preserves target score while lowering weighted, far-field, off-program, and harmful burden.
(d) Selector operating points compared with the observed-field oracle.
(e) Perturb-FISH provides module-level external support under a different action pool and readout panel.
(f) Predicted target utility stratifies observed target qualification, but calibration remains incomplete, especially under strict guide holdout.}
\label{fig:app_selector_prioritization}
\end{figure*}

Three pre-specified biological cases give the most interpretable view of the audit (Table~\ref{tab:app_selector_cases}). The selected alternatives retained 89.3--99.8\% of target utility while reducing collateral burden by 24.4--49.7\%. For TGF-$\beta$/stroma near-down, the selector replaced \texttt{sgrna\_rraga} with \texttt{sgrna\_oligo2}, preserving target score from 5.074 to 4.968 while reducing weighted burden from 1.188 to 0.597. The same case reduced far-field burden from 1.213 to 0.604, off-program burden from 1.228 to 0.615, and harmful burden from 1.457 to 0.778 (Table~\ref{tab:app_selector_burden_decomposition}). Vascular-stress near-down and myeloid-activation far-up show the same target-retaining, lower-burden pattern.

\begin{table*}[t]
\centering
\small
\caption{Pre-specified response-field prioritization cases. Target retention and burden reduction are computed relative to the target-only guide for the same target specification.}
\label{tab:app_selector_cases}
\begin{tabular}{lllrr}
\toprule
Biological target & Target-only guide & Selector guide & Retention & Burden red. \\
\midrule
TGF-$\beta$/stroma near-down & \texttt{sgrna\_rraga} & \texttt{sgrna\_oligo2} & 97.9\% & 49.7\% \\
Vascular stress near-down & \texttt{sgrna\_rraga} & \texttt{sgrna\_clu} & 99.8\% & 24.4\% \\
Myeloid activation far-up & \texttt{sgrna\_Cfap410} & \texttt{sgrna\_stk39} & 89.3\% & 24.4\% \\
\bottomrule
\end{tabular}
\end{table*}
...
\centering
\small
\caption{Burden decomposition for the three response-field prioritization cases. Scores are observed held-out field summaries. Lower burden is better.}
\label{tab:app_selector_burden_decomposition}
\begin{tabular}{llrrrrr}
\toprule
Target & Role & Target score & Weighted burden & Far-field & Off-program & Harmful \\
\midrule
...
\centering
\small
\caption{Selector operating points used only as a retrospective appendix audit. All rows use the same Spatial Perturb-seq target-specification family; operating points differ in burden definition or aggregation.}
\label{tab:app_selector_operating_points}
\begin{tabular}{lrrrr}
\toprule
Operating point & Candidate actions & Specs & TQR & Burden reduction \\
\midrule
Clean weighted-all head & 5,286 & 36 & 0.639 & 0.354 \\
Off-program sensitivity & 5,286 & 36 & 0.667 & 0.362 \\
Rank-consensus operating point & 5,286 & 36 & 0.620 & 0.418 \\
Observed-field oracle & 5,286 & 36 & 1.000 & 0.672 \\
\bottomrule
\end{tabular}
\end{table*}

Perturb-FISH provides external support for the direction of these response-field prioritization cases, but we interpret it as module-level support rather than full selector replication. The action pool and readout panel differ from Spatial Perturb-seq, so the external assay tests whether the same module-level target and burden directions are plausible rather than whether the identical guide-level selector transfers unchanged. On 39 reliable Perturb-FISH specifications, the full predicted-utility/predicted-burden selector reached TQR 0.718, burden reduction 0.151, observed-utility/predicted-burden reduction 0.207, and pairwise safety 0.851 (Table~\ref{tab:app_selector_perturb_fish}).

\begin{table*}[t]
\centering
\small
\caption{Perturb-FISH module-level external support for the response-field prioritization audit. The external assay has a different action pool and readout panel, so these rows support module-level direction rather than full selector replication.}
\label{tab:app_selector_perturb_fish}
\begin{tabular}{lrrrrr}
\toprule
Subset & Specs & Full TQR & Full burden red. & Obs-U/pred-S red. & Pairwise safety \\
\midrule
...
Finally, the audit has a calibration limitation. Predicted utility deciles do stratify observed target qualification, but absolute reliability is incomplete: the main block split rises from TQR 0.043 in the bottom predicted-utility decile to 0.575 in the top decile, while strict guide holdout rises from 0.053 to 0.461 (Table~\ref{tab:app_selector_calibration}). We therefore report the selector as a response-field prioritization audit and not as a calibrated controller for prospective intervention design.
