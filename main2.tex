...
\title{\methodname{}: Disentangling Tissue Context and Perturbation-Conditioned Response Fields in Spatial Genomics}
...
\author{Jing Zhang}
\affiliation{%
  \department{Department of Computer Science, Donald Bren School of Information and Computer Sciences}
  \institution{University of California, Irvine}
  \city{Irvine}
  \state{California}
  \country{USA}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\linewidth]{pair_2.png}
...
Experiments cover response-field prediction, benchmark evaluation, robustness,
and two scientific use cases.
...
test every guide once. Non-overlapping tissue blocks prevent anchors, matched
controls, or pseudo-targets from crossing partitions. Spatial seeds test
...
graph; uncertainty resamples blocks or guides, never cells as independent
replicates.
...
Spatial Perturb-seq determines cell-resolved claims; controlled LR propagation,
Perturb-map, and Perturb-FISH test known-rule recovery, spot/lesion transfer,
and native-panel burden ranking. Supporting assays probe failure modes but are
not pooled; the appendix reports full settings and resampling units.
...
On the exact-task split, \methodname{} outperforms the implemented statistical,
retrieval, radial, linear, patch, and generic graph alternatives. It reaches
Neighbor/Ring Pearson 0.684/0.457 and Neighbor MAE 0.408
...
(\Cref{fig:main_exp,tab:main_results}). Across 928 test graphs and 512 genes,
representative perturbation-related genes range from
...
far-field or off-program responses. We retain actions that satisfy a
validated local-program threshold and minimize far-field, off-program, and
undesirable-program burden. The utility constraint prevents inactive actions
from appearing favorable because all responses are small. Stress or apoptosis
terms represent toxicity-associated transcriptomic burden, not organismal
toxicity. Validation fixes all thresholds, weights, and operating points.
...
qualification while lowering collateral response burden.} Across 3,107
spatially separated actions and 36 target specifications, a validation-fixed
five-candidate consensus matches target-only TQR (0.6944 versus 0.6944) and
reduces mean spillover by 12.0\%
(\Cref{fig:drug_selector_application}). Absolute burden decreases by
...
for a context MLP. Burden falls for 24 specifications, rises for seven, and is
unchanged for five. The absolute interval excludes zero, whereas the relative
interval
...
ranking misses. Because these cases were explored post hoc, they explain the
mechanism but are not confirmatory estimates.
...
respectively (\Cref{tab:app_chemical_selector}). These observed-response audits
support the decision problem, not end-to-end spatial drug-field prediction,
treatment efficacy, or clinical safety.
\FloatBarrier
...
non-spatial direct-effect model is available.} On an intact mouse-brain graph
without paired IFN-$\gamma$ labels, CPA trained or fine-tuned on PerturbSapiens
predicts IFN-$\gamma$ and control states for each Sample2/pre4 spatial cell
using treatment status (PBS/control versus IFN-$\gamma$), cell type, and tissue
or broad class as covariates. Their difference defines a per-cell direct-effect
delta passed to \methodname{} propagation. This demonstrates the query
interface and generates hypotheses; it is not matched spatial validation.
...
Endothelial and pericyte states are also mildly enriched, concentrating the
hypothesis in immune, vascular, and glial microenvironments rather than
...
\Cref{tab:app_ifng_direct_effect_seed,app:ifng_extended_interpretation};
spatial perturbation remains necessary for validation.
\FloatBarrier
\section{Discussion and Conclusion}
\label{sec:discussion_conclusion}
Tissue context explains most global response-field structure, whereas
conditioning on perturbation contributes a smaller, reproducible radial
component. The lift survives gene-disjoint matching, requires local controls,
depends on direct-effect profile and sender location, and peaks near 64 cells
(98\,$\mu$m). It transfers to unseen guides only with assay-matched spatial
seeds; external scRNA seeds underperform zero seed. These controls support
local response-field prediction, not causal communication inference.

Pooled correlation rewards anatomy; graph-paired Ring lift tests
perturbation-specific reorganization beyond context. Their disagreement
identifies direct-effect transfer and top-action calibration as bottlenecks.

The validation-frozen selector preserves target qualification while modestly
reducing held-out burden. The utility--burden interface ranks drug--dose--time
actions in observed LINCS, Tahoe-100M, and Sci-Plex profiles. We therefore view
\methodname{} as a response-field selector for prioritization. Evidence
remains retrospective; undesirable-program scores are
transcriptomic proxies, not clinical safety estimates. Prospective studies
require preselected actions, assay-compatible seeds, and independent target
and collateral measurements.
...
Primary estimates come from one mouse-brain assay; scale and calibration may
not transfer across platforms. Guide-OOD gains are
modest; external scRNA seeds fail. Chemical-response audits validate the
decision formulation with observed
profiles, not end-to-end spatial prediction. Prospective validation is
required; public data remain subject to original terms.

\section{Generative AI Usage}
Generative AI was used only for language polishing and limited code
assistance. All figures were drawn manually by the authors. Generative AI was
not used for benchmark construction, experimental design, result generation,
...
module support, seed-transfer diagnostics, and calibration. This order keeps
...
Finally, predicted utility deciles stratify observed target qualification on
the formal main test block, where TQR rises from 0.043 to 0.575 across extreme
deciles (Table~\ref{tab:app_selector_calibration}). This result supports
utility ranking but does not assign a calibrated probability interpretation to
the score.

\begin{table}[t]
\centering
\small
\caption{Target-utility stratification on the formal main test block.
Target-qualified rate (TQR) increases from the lowest to highest
predicted-utility decile.}
