...
Experiments test claims in order: reconstruction, constrained selection, guide
and assay transfer, controlled perturbation signal, and qualitative deployment.

\subsection{Evaluation Design Separates Benchmarking from Confirmatory Effects}
\label{sec:benchmark_metrics}
Separate splits prevent development performance from being mistaken for
perturbation-specific evidence. Mouse-brain Spatial Perturb-seq is
the primary benchmark. The exact-task comparison uses the original development
split; perturbation estimates use a separate spatial split with 1,342 training,
837 validation, and 928 test graphs. Five grouped guide-out folds
assign each guide to test once. The three spatial partitions correspond to
non-overlapping tissue blocks, so no post anchor, matched-control patch, or
derived pseudo-target crosses train, validation, and test. Observed spatial
seeds test conditional propagation transfer; companion scRNA seeds test the
end-to-end cross-assay interface. Unless noted, runs use 64-cell patches on one
RTX A6000.

The metrics distinguish global reconstruction from radial perturbation
organization. Neighbor Pearson measures the complete signed receiver field,
centered Pearson discounts graph-level offsets, Ring Pearson tests organization
by distance from the direct-effect region, and Neighbor MAE measures absolute
error. Mechanistic claims compare context and perturbation-conditioned
predictions on the same graph, with uncertainty obtained by resampling spatial
blocks or guides. Cells within a graph are never treated as independent
biological replicates.

Finally, the datasets form an explicit evidence hierarchy. Spatial Perturb-seq
alone determines exact cell-resolved claims; a controlled ligand--receptor
benchmark tests recovery of a known rule, Perturb-map tests spot/lesion program
transfer, and Perturb-FISH tests native-panel burden ranking. These supporting
assays probe distinct failure modes and are not pooled to inflate sample size;
full settings and resampling units are reported in the appendix.
...
\subsection{Predicted Fields Enable Target-Retaining, Lower-Burden Drug and Perturbation Selection}
\label{sec:drug_selector_application}
Predicted response fields support constrained drug and perturbation selection.
The selector retains a requested local program response while minimizing far-field,
off-program, and undesirable-program burden. The hard utility threshold
prevents an inactive action from appearing favorable merely because all its
responses are small. When undesirable programs encode stress or apoptosis,
the objective is a toxicity-associated transcriptomic burden rather than
organismal toxicity. All thresholds, weights, and operating points are selected
on validation data before test evaluation.
...
The frozen selector preserves target qualification while reducing collateral
burden. Across 3,107 spatially separated actions and 36 target specifications,
the validation-selected five-candidate consensus matches target-only TQR
(0.6944 versus 0.6944) and reduces mean spillover by 12.0\%
(\Cref{fig:drug_selector_application}). Absolute burden decreases by 0.1244
[0.0559, 0.1926], regret falls from 0.4668 to 0.3424, and Safe@5 rises from
0.1111 to 0.4167. Thus, the utility constraint retains the requested effect
while burden ranking improves the selected action. This gain is not reproduced
by generic alternatives: random observed-qualified selection and Path2
aggregation yield mean reductions of $-9.8\%$ and $-16.4\%$, respectively,
whereas the dual utility/burden selector reaches $+12.9\%$
(\Cref{tab:app_selector_formal}).

The ranking signal is real but top-action calibration remains limiting. The
\methodname{} context spillover head reaches pairwise lower-burden agreement
0.6250 [0.6108, 0.6393], compared with 0.4909 for a context MLP. Selected
actions have lower burden for 24 specifications, higher burden for seven, and
unchanged burden for five. The absolute-decrease interval excludes zero,
whereas the relative-reduction interval [$-0.0018$, 0.2289] crosses zero
because ratios are unstable for specifications with small target-only burden.
The observed-field oracle reaches 60.6\% reduction. Pairwise ranking is
therefore learnable, but utility qualification and extreme-rank calibration
leave a large oracle gap.

Finally, the earlier 35.4\% reduction and the TGF-$\beta$/stroma,
vascular-stress, and myeloid-activation examples were identified during
discovery. These examples retain 89.3--99.8\% of target utility while reducing
weighted burden by 24.4--49.7\%, illustrating how similar target effects can
produce different far-field, off-program, and undesirable-program responses
(\Cref{fig:drug_selector_application}). Because both the operating family and
cases were explored post hoc, they illustrate the decision mechanism but are
not confirmatory effect estimates.

\subsection{Assay-Matched Seeds Transfer Across Guides, whereas Cross-Assay Seeds Do Not}
\label{sec:ablations}
...
Accurate context reconstruction alone does not guarantee perturbation-response
...
\subsection{Perturbation Signal Survives Decoupling but Depends on Local Scale}
...
\end{figure}
\subsection{IFN-\texorpdfstring{$\gamma$}{gamma} Deployment Produces a Spatially Heterogeneous Hypothesis}
\label{sec:ifng_case_study}
The IFN-$\gamma$ deployment produces a spatially heterogeneous, biologically
plausible hypothesis rather than a validated prediction. A CPA model trained on
PerturbSapiens supplies an external direct-effect profile for an unperturbed
mouse-brain graph, yielding heterogeneous IFN/MHC-I and structural responses
(\Cref{fig:ifng_response_field}). Canonical interferon genes such as
\textit{B2m} and \textit{Ifitm3} increase, whereas structural programs marked
by \textit{Col1a2} and \textit{Vcan} decrease across selected local patches.
Because this tissue has no paired spatial IFN-$\gamma$ ground truth, the case
demonstrates the query interface only; seed construction and numerical readouts
