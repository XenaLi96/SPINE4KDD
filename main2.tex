...
responses conflate stable tissue context with perturbation-conditioned effects.
We construct Pre--Post Perturbation Pairing (\ppp), an audited matched-control
pseudo-target benchmark, and \methodname{}, a predictive decomposition that
first estimates context-explainable response and then models the residual
conditioned on a direct-effect profile and sender region. On a fully disjoint
...
(95\% CI, 0.0474--0.0586). Context-only and full models attain predictive
...
retain positive Ring lift, whereas removing spatial matching does not,
indicating that the signal is not explained solely by expression overlap but
depends on spatially matched controls. Factorial controls show contributions
from both direct-effect profile identity and sender location, while randomized
communication scores have no measurable effect. Across five guide-OOD folds,
...
identifying cross-assay seed transfer as the current end-to-end bottleneck.
...
0.1244 (0.0559--0.1926). These results support context-dominated global fields,
perturbation-structured local organization, and modest retrospective
lower-burden prioritization rather than causal or clinical prediction.
\end{abstract}
...
Spatial functional genomics places perturbations within intact tissue and
measures molecular changes in situ
...
propagation. Spatial variance and multiview models show that intrinsic and
environmental components explain substantial spatial expression variation
...
We ask four linked questions. How much neighbor response is predictable from
tissue context, and where does perturbation signal remain? Does this residual
transfer across regions, guides, and assays? Can predicted fields prioritize
actions that preserve a local response while reducing collateral burden?
...
validation-frozen selector preserves target qualification and reduces held-out
burden by 12.0\%.
...
\begin{itemize}
    ...
    \item \textbf{A formal perturbation-prioritization task.}
    We evaluate target-retaining lower-burden selection on frozen validation
    and test partitions, while treating earlier biological cases as exploratory.
\end{itemize}
...
exact-task suite uses the original full-development comparison split; all
perturbation-specific effect estimates are independently re-estimated on the
strict 1,342/837/928 three-way spatial split. Five grouped guide-out folds
assign every guide to test once. Observed spatial seeds evaluate conditional
propagation transfer, whereas companion scRNA seeds test the end-to-end
cross-assay interface. Supporting analyses use a controlled ligand--receptor
benchmark, Perturb-map, and Perturb-FISH. We report Neighbor, centered, and
Ring Pearson together with Neighbor MAE. Unless noted, runs use 64-cell patches
on one RTX A6000; full settings and resampling units are in the appendix.

\textbf{Estimands and statistical units.}
The exact-task comparison asks how accurately a model reconstructs the complete
signed receiver field on a common held-out output. Neighbor correlation is
sensitive to stable anatomical and cell-state structure; centered correlation
discounts graph-level offsets; and Ring correlation tests whether response
magnitude is organized correctly with distance from the direct-effect region.
For mechanistic claims, we therefore compare perturbation-conditioned and
context predictions on the same held-out graph rather than subtracting two
pooled correlations. Uncertainty for graph-paired effects is computed by
resampling spatial blocks or guides, as appropriate. Cells within a graph are
not treated as independent biological replicates.

\textbf{Evidence hierarchy.}
The mouse-brain assay is the only exact, cell-resolved benchmark and determines
the confirmatory claims. The synthetic ligand--receptor benchmark tests whether
the architecture can recover a known propagation rule under controlled shifts.
Perturb-map tests program-level transfer after aggregation to spots and lesions,
whereas Perturb-FISH tests native-panel burden ranking under a different action
pool. These supporting assays address complementary failure modes and are not
used to increase the effective sample size of the primary benchmark.
...
On the original exact-task comparison split, \methodname{} reaches
Neighbor/Ring Pearson 0.684/0.457 with Neighbor MAE 0.408
(\Cref{fig:main_exp,tab:main_results,tab:main_baseline_comparison}). It
outperforms the implemented statistical, retrieval, radial, linear, patch, and
generic graph alternatives on the common cell-by-gene output. Gene-level
recovery is strongest for high-change and high-variance receiver genes, whereas
perturbation-related genes remain heterogeneous. Because this comparison uses
the full development set, we treat it as model benchmarking rather than an
estimate of perturbation-specific effect size.

The supporting assays probe different outputs. In controlled synthetic
ligand--receptor propagation, context-only Neighbor/Ring correlations are
0.003/0.006 and fine-tuning raises them to 0.619/0.815, showing learnability
when the response rule is known. Perturb-map provides coarser external support:
spot-level program Pearson reaches 0.853, 0.757, and 0.714 for
\textit{Ifngr2}, \textit{Jak2}, and \textit{Tgfbr2}, respectively. These
supporting results are not pooled with the strict mouse-brain effect estimates.

\subsection{Signal Decomposition and Benchmark Robustness}
\label{sec:signal_decomposition}
The strict three-way split separates absolute field prediction from the
increment attributable to perturbation conditioning. \Cref{tab:confirmatory_analysis}
collects the corresponding variance, matching, input-control, and
scale diagnostics.
...
\begin{table*}[t]
\centering
\caption{\textbf{Confirmatory signal decomposition, benchmark decoupling, and
robustness diagnostics.} Panels A, C, and D use the strict three-way spatial
split. Pooled Ring correlation and graph-paired lift are different estimands.
Partial $R^2=1-\mathrm{SSE}_{\mathrm{full}}/
\mathrm{SSE}_{\mathrm{context}}$.}
...
\textbf{A. Independent spatial split and predictive variance}
\vspace{1pt}
...
\multicolumn{5}{l}{\rule{0pt}{0.96em}} \\[-2.4pt]
\multicolumn{5}{l}{\rule{0pt}{0.96em}} \\[-2.4pt]
\bottomrule
\end{tabular*}
\end{minipage}
...
\midrule
Gene-disjoint match/output genes & $+0.0257$ & [0.0215, 0.0302] \\
No center-expression match & $+0.0281$ & [0.0244, 0.0319] \\
No patch-expression match & $+0.0263$ & [0.0226, 0.0300] \\
Strict no-expression match & $+0.0128$ & [0.0088, 0.0166] \\
No spatial match & $+0.0025$ & [$-0.0022$, 0.0066] \\
\bottomrule
\end{tabular*}
\end{minipage}
...
\setlength{\tabcolsep}{2.2pt}
\renewcommand{\arraystretch}{0.96}
\textbf{B. Direct-effect profile/location factorial}
\vspace{1pt}
...
\multicolumn{4}{l}{\rule{0pt}{0.96em}} \\[-2pt]
\bottomrule
\end{tabular*}
\end{minipage}
\vspace{3pt}
\parbox{0.98\linewidth}{\scriptsize\textit{Reading.}
Expression-decoupled variants retain positive lift, whereas no-spatial
matching does not. Both direct-effect profile and location contribute, while
larger patches have higher context predictability but not stronger
perturbation-conditioned signal.}
\end{table*}

\textbf{Independent spatial validation and predictive variance.}
On the fully separated train/validation/test split, adding the
perturbation-conditioned branch raises pooled Ring Pearson from 0.3626 to
0.4111. More importantly, graph-paired evaluation gives a mean Ring lift of
$0.0532$ with a spatial-block bootstrap interval that excludes zero. The same
comparison increases predictive $R^2$ from 0.3712 to 0.4669, corresponding to
a partial $R^2$ of 0.1521 relative to the context model. Thus, tissue context
accounts for most predictable variation, while perturbation conditioning adds
a smaller but reproducible component concentrated in radial organization.

\textbf{Benchmark decoupling.}
Positive graph-paired Ring lift remains when the genes used for matching are
disjoint from the 512 prediction genes, when either center or patch expression
is removed from matching, and under strict matching without expression. The
effect is attenuated as matching information is removed, and the no-spatial
variant is indistinguishable from zero. This pattern argues against direct
matching/output leakage as the sole explanation, while showing that spatially
local controls are essential for constructing an informative pseudo-target.

\textbf{Profile and location.}
The profile/location factorial separates two sources of conditioning.
Shuffling the direct-effect profile at the correct region reduces Ring
correlation from 0.4574 to 0.4056, whereas relocating the correct profile
reduces it to 0.4314. Both molecular identity and spatial placement therefore
contribute. Randomizing the communication prior leaves the score essentially
unchanged (0.4575), so the present evidence supports a learned response-field
dependency but does not support attributing the gain specifically to the
hand-specified communication prior.

\textbf{Spatial scale.}
The perturbation-conditioned lift is positive for 32- and 64-cell patches,
with the largest estimate at 64 cells (median radius 98.18\,$\mu$m), but becomes
negative for 100- and 128-cell patches. Larger patches are easier for the
context model in absolute correlation, yet they dilute the local
perturbation-specific organization. We therefore use 64 cells as an empirical
operating scale for this assay rather than as a universal biological radius.
...
positive in all five folds. This supports conditional propagation transfer
given an assay-matched direct-effect profile, but the magnitude is modest.

\textbf{Cross-assay seed failure.}
Replacing the spatial seed with a companion external scRNA-derived profile
reduces Ring Pearson from 0.4400 for zero seed to 0.4130, with all five folds
below zero seed. The current end-to-end bottleneck is therefore not spatial
routing alone, but cross-assay transfer of the direct-effect profile.

\textbf{Related-method diagnostic.}
Using the official CONCERT implementation, native reconstruction is strong,
but perturbation pseudo-delta recovery is below a guide-mean baseline in all
four evaluated contrasts. We interpret this as an objective mismatch rather
than a definitive method ranking: accurate context reconstruction does not
necessarily imply perturbation-response prediction. The original exact-task
input controls remain in \Cref{tab:seed_shortcut_controls}; full per-fold and
related-method values are reported in the appendix.
\begin{figure}[t]
\centering
...
\end{figure}
\subsection{De novo IFN-\texorpdfstring{$\gamma$}{gamma} Mouse-Brain Hypothesis}
\label{sec:ifng_case_study}
We retain the original IFN-$\gamma$ deployment as a hypothesis-generation
example. A CPA model trained on PerturbSapiens supplies
an external direct-effect profile for an unperturbed mouse-brain graph.
\Cref{fig:ifng_response_field} shows heterogeneous IFN/MHC-I and structural
response patterns, but no paired spatial IFN-$\gamma$ ground truth exists for
this tissue. The panel therefore illustrates the query interface and
biological plausibility only; it is not evidence for cross-assay predictive
accuracy. Seed construction and numerical readouts are in the appendix.

\subsection{Spatially Constrained Perturbation Prioritization}
\label{sec:drug_selector_application}
Spatial response fields define a retrospective decision problem: among actions
predicted to retain a requested local program response, select the action with
lower far-field, off-program, and undesirable-program burden. Thresholds and
weights are selected on validation data and frozen before evaluation.
...
burden-order agreement 0.6250 [0.6108, 0.6393], supporting a learnable ranking
signal. The effect is positive but moderate.

The selected action has lower observed burden for 24 of 36 target
specifications and higher burden for seven. The bootstrap interval for the
absolute decrease excludes zero, whereas the relative-reduction interval
($-0.0018$ to 0.2289) crosses zero because specifications with small baseline
burden yield unstable ratios. We therefore use absolute burden change as the
primary confirmatory effect and report the 12.0\% relative reduction as an
interpretable secondary summary. Pairwise agreement is stronger than the final
selection effect, indicating that spillover ordering is learnable but utility
qualification and top-action calibration still limit end-to-end decisions.
...
post-hoc illustrations and do not use them as confirmatory effect estimates.
Perturb-FISH calibration and module-level support remain in the appendix.

In Perturb-FISH, whose targeted panel and context-binned actions differ from the
primary benchmark, the same decision question is tested at module level. On 39
reliability-filtered specifications, full predicted-utility/predicted-burden
selection reaches TQR 0.718 and observed burden reduction 0.151. Holding target
qualification to observed utility and ranking only by predicted burden raises
the reduction to 0.207, while pairwise burden-order agreement is 0.851. Results
over all 64 specifications remain directionally positive but weaker. We treat
this as external support for burden ranking, not as evidence that a
cell-resolved site selector transfers unchanged to a panel-based assay.
\FloatBarrier
...
profile/location controls narrow the interpretation: expression overlap alone
does not explain the lift, spatially local pseudo-controls remain necessary,
and both sender profile and location matter. \methodname{} operationalizes this
distinction with a frozen context predictor and direct-effect-to-receiver
residual routing. Conditional guide-OOD transfer is positive when an
assay-matched spatial seed is available, but external scRNA seed failure shows
that end-to-end transfer remains unresolved. The formal selector converts the
predicted field into a modest, validation-frozen lower-burden decision benefit.

These results also clarify which evaluations answer which scientific question.
The original exact-task benchmark measures whether the complete response field
can be reconstructed under the same output definition used by all implemented
alternatives. The strict decomposition asks the narrower question of whether
the correct direct-effect input improves radial organization beyond context.
The guide-OOD experiment asks whether that conditional mapping transfers to
unseen perturbations, and the selector asks whether the remaining signal is
decision-relevant. Agreement across these estimands is more informative than a
single pooled Pearson score, while disagreement identifies the current
bottleneck rather than being hidden by an aggregate benchmark.

The spatial-scale result provides a concrete biological and modeling
interpretation. Absolute context predictability rises in larger neighborhoods,
but perturbation-conditioned lift peaks near the 64-cell, 98-$\mu$m setting and
then reverses. The model is therefore most informative over a local field in
which sender identity can still organize receiver responses; expanding the
patch eventually adds anatomy faster than perturbation-specific signal. This
observation motivates reporting physical radius together with cell count and
testing scale anew for every assay.

Finally, the intervention analysis is a ranking result rather than a treatment
recommendation. It shows that predicted fields contain enough ordering
information to preserve the target-qualified rate while reducing held-out
absolute burden under one validation-frozen operating point. The moderate
effect, failed external-scRNA seed transfer, and imperfect relative-reduction
interval all argue for the same next step: prospective spatial experiments in
which candidate actions are chosen before measurement and both target response
and collateral field are assayed independently.

The confirmatory controls define clear failure conditions for the proposed
interpretation. If positive lift had vanished under gene-disjoint or
no-expression matching, the result would have been compatible with matching
leakage. If the no-spatial construction had retained the same lift, local
counterfactual matching would not have been necessary. If randomized
communication scores had reduced performance, the curated edge prior could
have been credited as a mechanism; it did not. Finally, if external scRNA seeds
had matched assay-derived seeds, the present system could have supported
end-to-end unseen-perturbation claims; instead they underperform zero seed.
Together, these outcomes support the narrower claim of assay-conditioned local
response-field prediction and identify direct-effect transfer as a separate,
unsolved problem.

For practical use, the current interface should begin with an
assay-compatible direct-effect measurement, not perturbation identity alone.
Candidate actions should be compared within the same tissue and acquisition
regime, with target qualification enforced before burden ranking. Actions for
which utility or burden is poorly calibrated should be deferred rather than
forced into a ranking. This scope still supports retrospective experiment
prioritization: it can identify matched-utility actions whose measured spatial
fields warrant prospective comparison, while avoiding unsupported claims about
synthetic interventions or unmeasured tissues.
...
depend on local control states. The 64-cell setting maximizes paired lift in
this mouse-brain assay, but its approximately 98-$\mu$m scale may not transfer
across tissues, platforms, or cell densities. Guide-OOD gain from
assay-matched spatial seeds is modest, and companion scRNA seeds underperform
zero seed. Cached communication scores and the reference-ST adapter provide no
robust gain and are not core components. The formal selector yields a positive
but moderate 12.0\% reduction; larger discovery-set reductions and individual
biological cases are post-hoc illustrations. All predictions and selector
results are retrospective, not causal or clinical, and require prospective
spatial perturbation validation. Public or de-identified molecular and spatial
data should remain governed by their original consent and data-use terms.
The primary effect estimates come from one mouse-brain platform and should not
be read as evidence of organ-wide generalization. Graphs within a slide share
anatomy and experimental history; spatial-block and guide-level resampling
reduce, but do not eliminate, dependence. Program definitions and undesirable
burden components also encode scientific choices that must be pre-specified for
each new study. Perturb-FISH supports module-level ordering under a targeted
panel, but cannot validate transcriptome-wide cell-level fields.
...
We will release code, configurations, processed split manifests, benchmark
construction scripts, machine-readable result tables, and evaluation scripts
in a public repository, with the final URL added before submission. The release
will include PPP control assignments, strict spatial and guide-held-out
partitions, direct-effect input controls, patch-radius manifests, selector
target specifications, frozen validation choices, and scripts that reproduce
all confidence intervals in \Cref{tab:confirmatory_analysis}. Model checkpoints
and derived response-field summaries will be provided where source-data terms
permit. Raw data remain available from their original public repositories under
the cited accession records.
\clearpage
...
\subsection{Exploratory Retrospective Perturbation-Prioritization Audit}
\label{app:selector_audit}

Beyond predicting spatial response fields, we asked whether frozen
\methodname{} outputs could prioritize perturbations that preserve a specified
target program in a spatial Ring while reducing collateral burden. The
operating points and cases below were identified in the earlier discovery
analysis. They are retained only as exploratory post-hoc illustrations and are
not confirmatory effect estimates.

The exploratory analysis scores target utility and collateral burden, including
far-field, off-program, and undesirable-program components. Across 5,286
candidate actions and 36 target specifications, the discovery operating points
reached reductions of 0.354 and 0.418; the observed-field oracle reached 0.672.
These values must not be interpreted as validation-frozen selector estimates.
...
Perturb-seq. On 39 reliable specifications, predicted-utility/predicted-burden
selection reached TQR 0.718, burden reduction 0.151,
observed-utility/predicted-burden reduction 0.207, and pairwise burden-order
agreement 0.851 (Table~\ref{tab:app_selector_perturb_fish}).
...
this as a retrospective prioritization audit, not a calibrated prospective
controller.
