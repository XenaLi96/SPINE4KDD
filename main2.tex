...
perturbation-structured local organization, and a response-field-based
drug/perturbation selector for target-retaining, lower-burden experimental
prioritization rather than causal or clinical safety prediction.
\end{abstract}
...
\setlength{\tabcolsep}{3.0pt}
\renewcommand{\arraystretch}{1.08}
\textbf{B. Direct-effect profile/location controls}
\vspace{1pt}
...
\subsection{Response-Field-Based Drug and Perturbation Selection}
\label{sec:drug_selector_application}
Spatial response fields define a drug/perturbation-selection problem: among
actions predicted to retain a requested local program response, select the
candidate with lower far-field, off-program, and undesirable-program burden.
The hard utility constraint prevents the selector from obtaining an apparently
low burden by choosing an inactive action. Where the undesirable programs
encode stress or apoptosis, this is a target-retaining,
toxicity-associated-burden selector. Thresholds, component weights, and the
operating point are selected on validation data and frozen before test
evaluation.
...
The confirmatory audit contains 3,107 actions in fully spatially disjoint
train/validation/test blocks and 36 target specifications. Validation selects
a five-candidate consensus operating point without access to the test block.
On frozen test data, the constrained selector preserves the target-qualified
rate (TQR) of target-only exactly (0.6944 versus 0.6944), while reducing mean
spillover by 12.0\%. Absolute burden decreases by 0.1244
[0.0559, 0.1926], spillover regret falls from 0.4668 to 0.3424, and Safe@5
rises from 0.1111 to 0.4167. The observed-field oracle reduces spillover by
60.6\%, showing both that lower-burden alternatives exist and that substantial
headroom remains.

A standalone \methodname{} context spillover head reaches pairwise
lower-burden agreement 0.6250 [0.6108, 0.6393], compared with 0.4909 for a
context MLP. The selected action has lower observed burden for 24 of 36 target
specifications, higher burden for seven, and unchanged burden for five. The
absolute-decrease interval excludes zero, whereas the relative-reduction
interval ($-0.0018$ to 0.2289) crosses zero because ratios are unstable when
baseline burden is small. We therefore use absolute burden change as the
primary confirmatory effect. The stronger pairwise signal than end-to-end
selection effect identifies target qualification and top-action calibration,
rather than burden ranking alone, as the main remaining bottlenecks.
...
Perturb-FISH tests the same decision rule with context-binned actions and
native-panel programs. On 39 reliability-filtered specifications, full
predicted-utility/predicted-burden selection reaches TQR 0.718 and observed
burden reduction 0.151. Holding qualification to observed utility and ranking
only by predicted burden raises the reduction to 0.207, while pairwise
lower-burden agreement reaches 0.851. Separate observed-response audits on
LINCS Phase I and II, Tahoe-100M pseudobulk, and Sci-Plex further show that the
same constrained interface can rank drug--dose--time actions across cellular
contexts. These chemical assays establish the applicability of the decision
formulation to drug selection once response fields are available; they are not
end-to-end spatial predictions by \methodname{}. Complete baselines,
calibration, guide-OOD stress tests, external-assay values, and exploratory
cases are reported in \Cref{app:selector_audit}.
