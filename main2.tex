...
\FloatBarrier
\section{Discussion, Conclusion, Limitations, and Ethical Considerations}
\label{sec:discussion_conclusion}
\label{sec:limitations_ethical_considerations}
Tissue context explains most global response-field structure, whereas
perturbation conditioning contributes a smaller, reproducible radial component.
The lift survives gene-disjoint matching, requires local controls, profile
identity, and sender location, and peaks near 64 cells (98-$\mu$m). It
transfers to unseen guides with assay-matched spatial
seeds, but external scRNA seeds underperform zero seed. Thus, \methodname{}
supports assayed response-field prediction, not causal communication inference.

Pooled correlation rewards stable anatomy, whereas graph-paired Ring lift tests
seed-dependent reorganization beyond context. Their disagreement highlights
direct-effect transfer and top-action calibration as bottlenecks. The
validation-frozen selector nevertheless preserves target
qualification while modestly reducing held-out burden. The same constrained
utility--burden interface supports retrospective drug--dose--time ranking in
LINCS, Tahoe-100M pseudobulk, and Sci-Plex. We therefore view \methodname{} as a
response-field-based drug and perturbation selector for experimental
prioritization, not treatment recommendation or clinical safety estimation.

Limitations include \ppp's matched pseudo-counterfactual, one primary
mouse-brain assay, modest guide-OOD gains, failed cross-assay transfer, and
retrospective selection. Spatial scale, program
definitions, and calibration may not transfer across tissues or platforms;
chemical audits validate the decision formulation, not end-to-end spatial
drug-field prediction. Prospective studies should preselect actions, use
assay-compatible seeds, and measure target and collateral fields independently.
Public data remain subject to their original consent and use terms.
